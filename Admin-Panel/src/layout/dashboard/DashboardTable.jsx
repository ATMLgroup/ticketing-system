...
export const DashboardTable = () => {
    ...
    const openTicketDetails = async (id) => {
        const {data} = await sendRequest(`admin/tickets?ticket_id=${id}`, "GET", {})
    ...
    const sendReply = async () => {
        if (reply) {
            ...
            const {data, error} = await sendRequest("admin/reply", "POST", body)
    ...
    const updateChats = async () => {
        const {data: ticketData} = await sendRequest(`admin/tickets?ticket_id=${modalId}`, "GET", {})
                        ...
                        <div style={{marginTop: "50px"}}>
                            ...
                            </Button>
                            {
                                modalStatus !== "Close" && (<Button
                                    ...
                                    onClick={() => closeTicket()}
                                    type={"primary"}
                                    style={{backgroundColor: "rgba(247,45,43)"}}>
                                    Close ticket
                                </Button>)
                            }
                            {
